...
Parameters are used to control the behavior of events at run time. In the [FMOD Studio](<https://fmod.com/docs/2.02/studio//glossary.html#fmod-studio>) application, parameters can be used to automate the properties of events. Parameter values can then be changed at run time using the FMOD Studio API, which causes the automated event properties to change as a result.

Parameters can exist locally or globally. Local parameters exist on a per-event basis; each event instance that uses a given parameter has a single instance of that parameter, the value of which is independent from all other instances of the same parameter. A global parameter only ever has a single instance, which is shared between all events that make use of it, as well as the mixer.
